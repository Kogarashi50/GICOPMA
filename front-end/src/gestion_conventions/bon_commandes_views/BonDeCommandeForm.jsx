    ...
    useEffect(() => {
        ...
        axios.get(`${baseApiUrl}/bon-de-commande/${itemId}`, { withCredentials: true })
            .then(response => {
                ...
                setExistingFichiers((data.fichiers || []).map(f => ({ ...f, url: `${STORAGE_URL}/${f.chemin_fichier}` })));
                setFichiers([]);
                setFichiersToDelete([]);
            })
            ...
            <div className="flex-grow-1 px-md-3">
                ...
                <Form noValidate onSubmit={handleSubmit}>
                    <Row className="mb-4 g-3">
                        <Col md={6}><Form.Group controlId="formNumeroBc"><Form.Label className=" mb-1 fw-medium">Numéro BC <span className="text-danger">*</span></Form.Label><Form.Control className={FORM_CONTROL_CLASS} isInvalid={!!formErrors.numero_bc} required type="text" name="numero_bc" value={formData.numero_bc} onChange={handleChange} size="sm" /><Form.Control.Feedback type="invalid">{formErrors.numero_bc}</Form.Control.Feedback></Form.Group></Col>
                        <Col md={6}><Form.Group controlId="formDateEmission"><Form.Label className=" mb-1 fw-medium">Date Émission <span className="text-danger">*</span></Form.Label><Form.Control className={FORM_CONTROL_CLASS} isInvalid={!!formErrors.date_emission} required type="date" name="date_emission" value={formData.date_emission} onChange={handleChange} size="sm" /><Form.Control.Feedback type="invalid">{formErrors.date_emission}</Form.Control.Feedback></Form.Group></Col>
                    </Row>
                    <Form.Group controlId="formObjet" className="mb-4"><Form.Label className=" mb-1 fw-medium">Objet <span className="text-danger">*</span></Form.Label><Form.Control as="textarea" rows={2} className={FORM_TEXTAREA_CLASS} isInvalid={!!formErrors.objet} required name="objet" value={formData.objet} onChange={handleChange} size="sm" /><Form.Control.Feedback type="invalid">{formErrors.objet}</Form.Control.Feedback></Form.Group>
                    <Row className="mb-4 g-3 ">
                        <Col md={4}><Form.Group controlId="formFournisseurNom"><Form.Label className=" mb-1 fw-medium">Fournisseur <span className="text-danger">*</span></Form.Label><Form.Control className={FORM_CONTROL_CLASS} isInvalid={!!formErrors.fournisseur_nom} required type="text" name="fournisseur_nom" value={formData.fournisseur_nom} onChange={handleChange} size="sm" /><Form.Control.Feedback type="invalid">{formErrors.fournisseur_nom}</Form.Control.Feedback></Form.Group></Col>
                        <Col md={4}><Form.Group controlId="formMontantTotal"><Form.Label className=" mb-1 fw-medium">Montant Total TTC <span className="text-danger">*</span></Form.Label><Form.Control className={FORM_CONTROL_CLASS} isInvalid={!!formErrors.montant_total} required type="number" step="0.01" min="0" name="montant_total" value={formData.montant_total} onChange={handleChange} size="sm" placeholder="0.00" /><Form.Control.Feedback type="invalid">{formErrors.montant_total}</Form.Control.Feedback></Form.Group></Col>
                        <Col md={4}><Form.Group controlId="formEtatBC"><Form.Label className=" mb-1 fw-medium">État</Form.Label><Form.Select className={FORM_SELECT_CLASS} name="etat" value={formData.etat} onChange={handleChange} isInvalid={!!formErrors.etat}><option value="">Sélectionner...</option>{etatOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</Form.Select></Form.Group></Col>

                    </Row>
                    <Row className="mb-4 g-3">
                        <Col md={6}><Form.Group controlId="formMarcheBC"><Form.Label className=" mb-1 fw-medium">Marché Associé</Form.Label><Select inputId="marche_bc_select" name="marche" options={marcheOptions} value={formData.marche} onChange={(opt) => handleSelectChange('marche', opt)} styles={selectStyles} placeholder="- Sélectionner -" isClearable classNamePrefix="react-select" className={formErrors.marche_id ? 'is-invalid' : ''} menuPortalTarget={document.body}/></Form.Group></Col>
                        <Col md={6}><Form.Group controlId="formContratBC"><Form.Label className=" mb-1 fw-medium">Contrat Associé</Form.Label><Select inputId="contrat_bc_select" name="contrat" options={contratOptions} value={formData.contrat} onChange={(opt) => handleSelectChange('contrat', opt)} styles={selectStyles} placeholder="- Sélectionner -" isClearable classNamePrefix="react-select" className={formErrors.contrat_id ? 'is-invalid' : ''} menuPortalTarget={document.body}/></Form.Group></Col>
                    </Row>
                    <Form.Group controlId="formFonctionnaireBC" className="my-3"><Form.Label className=" mb-1 fw-medium"><FontAwesomeIcon icon={faUsers} className="me-2 text-secondary" /> Points Focaux</Form.Label><Select inputId="fonctionnaires_bc_select" name="fonctionnaires" options={fonctionnairesOptions} value={formData.fonctionnaires} onChange={handleFonctionnaireChange} styles={selectStyles} placeholder="Sélectionner (Optionnel)..." isClearable isMulti closeMenuOnSelect={false} className={formErrors.id_fonctionnaire ? 'is-invalid' : ''} menuPortalTarget={document.body}/></Form.Group>
