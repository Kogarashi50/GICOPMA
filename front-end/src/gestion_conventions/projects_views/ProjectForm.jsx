// src/gestion_conventions/projects_views/ProjectForm.jsx (Final Version - Based on Code 1)
...
import {
    ...
    faUsers // Icon for Fonctionnaire label
...
// --- Styles & Classes ---
const selectStyles = { // Using the merged styles definition
    ...
    menu: (provided) => ({ ...provided, borderRadius: '0.5rem', boxShadow: '0 0.5rem 1rem rgba(0, 0, 0, 0.15)', zIndex: 1055 }), // Keep increased zIndex
    ...
    multiValueLabel: (provided) => ({ ...provided, color: '#495057', padding: '2px 5px', fontSize: '0.8rem' }), // Adjusted font size slightly
...
// Using Code 1's class constants
const FORM_CONTAINER_CLASS = "p-3 p-md-4 projet-form-container";
const inputClass = "form-control form-control-sm rounded-pill shadow-sm bg-light border"; // Consistent input style
const textareaClass = "form-control form-control-sm rounded-3 shadow-sm bg-light border"; // Consistent textarea style
const FORM_ACTIONS_ROW_CLASS = "mt-4 pt-2 justify-content-center flex-shrink-0";
const FORM_CANCEL_BUTTON_CLASS = "btn px-5 rounded-5 py-2 shadow-sm"; // Standardized button style
const FORM_SUBMIT_BUTTON_CLASS = "btn rounded-5 px-5 py-2 align-items-center d-flex justify-content-evenly border-0 shadow-sm"; // Standardized button style
const FORM_HEADER_CLOSE_BUTTON_CLASS = 'btn rounded-5 px-5 py-2 bg-warning shadow-sm fw-bold'; // Keep V1 close button style
// --- End Styles & Classes ---
...
const ProjetForm = ({
    ...
    // --- State (Including fonctionnaires) ---
    const initialFormData = useMemo(() => ({
        ...
        fonctionnaires: [], // Initial state for fonctionnaires
    }), []);
    ...
    const [fonctionnairesOptions, setFonctionnairesOptions] = useState([]); // State for fonctionnaire options
    ...
    const [loadingOptions, setLoadingOptions] = useState({ domaines: true, programmes: true, chantiers: true, conventions: true, partenaires: true, fonctionnaires: true }); // Include fonctionnaires loading
    ...
    const optionsFinishedLoading = useMemo(() =>
        !loadingOptions.domaines && !loadingOptions.programmes && !loadingOptions.chantiers && !loadingOptions.conventions && !loadingOptions.partenaires && !loadingOptions.fonctionnaires,
        [loadingOptions]
    );

    // --- Fetch Options (Includes Partenaires and Fonctionnaires) ---
    const fetchOptions = useCallback(async () => {
        setLoadingOptions({ domaines: true, programmes: true, chantiers: true, conventions: true, partenaires: true, fonctionnaires: true });
        try {
            const [domRes, progRes, chanRes, convRes, partRes, foncRes] = await Promise.all([ /* ... axios calls ... */
            ...
            setPartenaireOptions((partRes.data.partenaires || []).map(p => ({ value: p.Id, label: p.Description_Arr || p.Description || `Partenaire Code ${p.Code}` })));
            setFonctionnairesOptions((foncRes.data.fonctionnaires || []).map(f => ({ value: f.id, label: f.nom_complet || `ID ${f.id}` })).sort((a,b) => a.label.localeCompare(b.label))); // Process & sort fonctionnaires

        } catch (err) { console.error("Erreur chargement options:", err); setSubmissionStatus(prev => ({ ...prev, error: "Erreur chargement des listes." })); }
        finally { setLoadingOptions({ domaines: false, programmes: false, chantiers: false, conventions: false, partenaires: false, fonctionnaires: false }); }
    }, [baseApiUrl]);

    useEffect(() => { fetchOptions(); }, [fetchOptions]);

    // --- Fetch Existing Data (Includes Fonctionnaire logic) ---
    useEffect(() => {
        // Use derived optionsFinishedLoading state
        if (!isEditing || !optionsFinishedLoading) { setLoadingData(false); return; }
        let isMounted = true;
        const fetchProjetData = async () => {
            ...
            try {
                ...
                console.log("[Projet Form Edit Load] Raw Data:", data);
                ...
                const selectedFonctionnaireOptions = findMultiOptions(fonctionnairesOptions, data.id_fonctionnaire); // Find selected fonctionnaires

                setFormData({ // Set form data including fonctionnaires
                    Code_Projet: data.Code_Projet ?? '', Nom_Projet: String(data.Nom_Projet ?? ''),
                    Cout_CRO: data.Cout_CRO ?? '', Date_Debut: data.Date_Debut?.split('T')[0] ?? '',
                    Observations: data.Observations ?? '', Etat_Avan_Physi: data.Etat_Avan_Physi ?? '',
                    Date_Fin: data.Date_Fin?.split('T')[0] ?? '', Cout_Projet: data.Cout_Projet ?? '',
                    ...
                    fonctionnaires: selectedFonctionnaireOptions, // Set selected fonctionnaires
                });

                // Process engagements (assuming key is engagements_financiers)
                const fetchedEngagements = data.engagements_financiers || [];
                setEngagementsList(fetchedEngagements.map(eng => ({ id: safeParseInt(eng.id), tempId: generateTempId(), partenaire: findOption(partenaireOptions, eng.partenaire_id), montant_engage: String(eng.montant_engage ?? ''), date_engagement: eng.date_engagement?.split('T')[0] ?? '', est_formalise: !!eng.est_formalise, commentaire: eng.commentaire ?? '' })));

            } catch (err) { /* ... error handling ... */ if (isMounted) setSubmissionStatus({ loading: false, error: err.response?.data?.message || err.message || "Erreur chargement projet.", success: false }); }
            finally { if (isMounted) setLoadingData(false); }
        };
        fetchProjetData();
        return () => { isMounted = false; };
    }, [itemId, isEditing, baseApiUrl, optionsFinishedLoading, domaineOptions, programmeOptions, chantierOptions, conventionOptions, partenaireOptions, fonctionnairesOptions]); // Added fonctionnairesOptions


    // --- Reset Form (Create Mode - Includes Fonctionnaire & ESLint fix) ---
    useEffect(() => {
        if (!isEditing && optionsFinishedLoading) {
            console.log("ProjetForm: Resetting form for Create mode.");
            setFormData(initialFormData); // Use memoized initial state
            setEngagementsList([]);
            setCurrentEngagement({ partenaire: null, montant_engage: '', date_engagement: '', est_formalise: false, commentaire: '' });
            setFormErrors({}); setEngagementErrors({}); setSubmissionStatus({}); setLoadingData(false);
            setDataToResubmit(null); setShowConfirmModal(false);
        }
    }, [isEditing, optionsFinishedLoading, initialFormData]); // Added initialFormData dependency


    // --- Frontend Validation (Using V1's stricter logic including fonctionnaires) ---
    const validateForm = () => {
        const errors = {};
        ...
        // Keep convention optional for now, align with backend if needed
        // if (!formData.convention) errors.Convention_Code = "Convention requise.";
        // Keep dates optional for now
        ...
        const checkNumeric = (field, name) => { const v = formData[field]; if (v !== '' && v !== null && (isNaN(parseCurrency(v)) || parseCurrency(v) < 0)) errors[field] = `${name} doit être numérique positif.`; };
        checkNumeric('Cout_CRO', 'Coût CRO'); checkNumeric('Cout_Projet', 'Coût Projet');
        ...
        // --- Optional Fonctionnaire Validation (Example: required) ---
        ...
        // }
        // --- End Fonctionnaire Validation ---

        setFormErrors(errors);
        return Object.keys(errors).length === 0;
    };
    const validateCurrentEngagement = () => { /* ... same as before ... */ const errors = {}; if (!currentEngagement.partenaire) errors.partenaire = "Partenaire requis."; if (!currentEngagement.montant_engage || isNaN(parseCurrency(currentEngagement.montant_engage)) || parseCurrency(currentEngagement.montant_engage) < 0) errors.montant_engage = "Montant valide requis."; if (!currentEngagement.date_engagement) errors.date_engagement = "Date engagement requise."; setEngagementErrors(errors); return Object.keys(errors).length === 0; };


    // --- Handlers (Including Fonctionnaire Handler) ---
    const handleChange = (e) => { /* ... same as before ... */ const { name, value } = e.target; setFormData(prev => ({ ...prev, [name]: value })); if (formErrors[name]) setFormErrors(prev => ({ ...prev, [name]: undefined })); };
    const handleDomaineChange = (selectedOption) => { /* ... same ... */ setFormData(prev => ({ ...prev, domaine: selectedOption })); if (formErrors.Id_Domaine) setFormErrors(prev => ({ ...prev, Id_Domaine: undefined })); };
    const handleProgrammeChange = (selectedOption) => { /* ... same ... */ setFormData(prev => ({ ...prev, programme: selectedOption })); if (formErrors.Id_Programme) setFormErrors(prev => ({ ...prev, Id_Programme: undefined })); };
    const handleChantierChange = (selectedOption) => { /* ... same ... */ setFormData(prev => ({ ...prev, chantier: selectedOption })); if (formErrors.Id_Chantier) setFormErrors(prev => ({ ...prev, Id_Chantier: undefined })); };
    const handleConventionChange = (selectedOption) => { /* ... same ... */ setFormData(prev => ({ ...prev, convention: selectedOption })); if (formErrors.Convention_Code) setFormErrors(prev => ({ ...prev, Convention_Code: undefined })); };
    ...
    const handleEngagementChange = (e) => { /* ... same ... */ const { name, value, type, checked } = e.target; setCurrentEngagement(prev => ({ ...prev, [name]: type === 'checkbox' ? checked : value })); if (engagementErrors[name]) setEngagementErrors(prev => ({ ...prev, [name]: undefined })); };
    const handleEngagementPartnerChange = (selectedOption) => { /* ... same ... */ setCurrentEngagement(prev => ({ ...prev, partenaire: selectedOption })); if (engagementErrors.partenaire) setEngagementErrors(prev => ({ ...prev, partenaire: undefined })); };
    const handleAddEngagement = () => { /* ... same ... */ if (!validateCurrentEngagement()) return; if (currentEngagement.partenaire && engagementsList.some(eng => eng.partenaire?.value === currentEngagement.partenaire?.value)) { setEngagementErrors(prev => ({ ...prev, partenaire: "Partenaire déjà ajouté." })); return; } setEngagementsList(prev => [...prev, { id: null, tempId: generateTempId(), ...currentEngagement }]); setCurrentEngagement({ partenaire: null, montant_engage: '', date_engagement: '', est_formalise: false, commentaire: '' }); setEngagementErrors({}); if (formErrors.engagements) setFormErrors(prev => ({ ...prev, engagements: undefined })); };
    const handleRemoveEngagement = (tempIdToRemove) => { /* ... same ... */ setEngagementsList(prev => prev.filter(eng => eng.tempId !== tempIdToRemove)); };
    const handleModalConfirm = () => { /* V2 handler */ setShowConfirmModal(false); if (dataToResubmit) { executeSubmit(dataToResubmit, true); } else { console.error("Cannot resubmit confirmation."); setSubmissionStatus({ loading: false, error: "Erreur interne.", success: false }); } };
    const handleModalCancel = () => { /* V2 handler */ setShowConfirmModal(false); setDataToResubmit(null); console.log("User cancelled cascade delete."); };


    // --- Submit Handler (Includes Fonctionnaire in Payload) ---
    const executeSubmit = async (dataPayload, confirmDelete = false) => {
        setSubmissionStatus({ loading: true, error: null, success: false }); setFormErrors({}); setDataToResubmit(null);
        const url = isEditing ? `${baseApiUrl}/projets/${itemId}` : `${baseApiUrl}/projets`;
        const method = isEditing ? 'put' : 'post';
        const finalPayload = { ...dataPayload, ...(confirmDelete && { confirm_cascade_delete: true }) };
        const config = { headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, withCredentials: true };
        console.log(`Submitting ${method.toUpperCase()} to ${url}. Confirmed: ${confirmDelete}. Payload:`, finalPayload);

        try {
            const response = await axios({ method: method, url: url, data: finalPayload, headers: config.headers, withCredentials: config.withCredentials });
            console.log("API Response:", response.data);
            setSubmissionStatus({ loading: false, error: null, success: true });
            const returnedProjet = response.data.projet;
            if (isEditing) onItemUpdated?.(returnedProjet); else onItemCreated?.(returnedProjet);
            // Close form after delay
            setTimeout(onClose, 1500);
        } catch (err) {
             console.error(`Erreur ${isEditing ? 'modif.' : 'création'} (${method}):`, err.response || err);
             let errorMsg = `Une erreur s'est produite.`; let serverErrors = {};
             if (err.response) {
                 if (err.response.status === 409 && err.response.data?.requires_confirmation) { /* Handle confirmation */ setSubmissionStatus({ loading: false }); setConfirmModalData({ message: err.response.data.message || "Conf. requise.", details: err.response.data.details || [] }); setDataToResubmit(dataPayload); setShowConfirmModal(true); return; }
                 errorMsg = err.response.data?.message || `Erreur ${err.response.status}`;
                 if (err.response.status === 422 && typeof err.response.data.errors === 'object') { /* Map validation errors */ serverErrors = err.response.data.errors; const mapped = {}; let engErr = ''; Object.keys(serverErrors).forEach(k => { const msg = serverErrors[k].join(' '); if(k.startsWith('engagements.')) engErr += msg + ' '; else if (k === 'id_fonctionnaire') mapped['id_fonctionnaire'] = msg; /* Map fonctionnaire error */ else mapped[k] = msg; }); if(engErr) mapped['engagements'] = engErr.trim(); setFormErrors(mapped); errorMsg = "Erreurs de validation."; }
             } else if (err.request) { errorMsg = "Pas de réponse serveur."; } else { errorMsg = err.message; }
             setSubmissionStatus({ loading: false, error: errorMsg, success: false });
         }
    };
    // Prepare payload including fonctionnaires
    const handleSubmit = (e) => {
        e.preventDefault(); setShowConfirmModal(false);
        if (!validateForm()) { setSubmissionStatus({ loading: false, error: "Veuillez corriger les erreurs.", success: false }); return; }

        // Prepare fonctionnaire IDs string
        ...
        const dataToSubmit = {
            ...
            Cout_CRO: formData.Cout_CRO ? parseCurrency(formData.Cout_CRO) : null, // Send null if empty
            Date_Debut: formData.Date_Debut || null,
            Observations: formData.Observations ?? '',
            Etat_Avan_Physi: formData.Etat_Avan_Physi ? parseCurrency(formData.Etat_Avan_Physi) : null, // Send null if empty
            Date_Fin: formData.Date_Fin || null,
            Cout_Projet: formData.Cout_Projet ? parseCurrency(formData.Cout_Projet) : null, // Send null if empty
            ...
            id_fonctionnaire: fonctionnaireIdsString || null, // Send string or null
            engagements: engagementsList.map(eng => { let obj = { partenaire_id: safeParseInt(eng.partenaire?.value), montant_engage: eng.montant_engage ? parseCurrency(eng.montant_engage) : null, date_engagement: eng.date_engagement || null, est_formalise: eng.est_formalise, commentaire: eng.commentaire ?? '' }; const pId = safeParseInt(eng.id); if(isEditing && pId !== null) obj.id = pId; return obj; }) // Include id only if editing existing
        };
    ...
    return (
        <>
            ...
            <div className={FORM_CONTAINER_CLASS} style={{ backgroundColor: '#fff', borderRadius: '20px', boxShadow: '0 6px 18px rgba(0,0,0,0.1)', maxHeight: 'calc(90vh - 100px)', overflowY: 'auto' }}>
                ...
                <div className="flex-grow-1">
                    ...
                    <Form noValidate onSubmit={handleSubmit}>
                        {/* === Main Project Fields (Using V1 Style) === */}
                        ...
                        <Row className="mb-1 g-3">
                         ...
                         <Row className="mb-1 g-3">
                             <Form.Group as={Col} md={6} controlId="formDateDebut"><Form.Label className="small mb-1 fw-medium">Date Début</Form.Label><Form.Control className={inputClass} isInvalid={!!formErrors.Date_Debut} type="date" name="Date_Debut" value={formData.Date_Debut} onChange={handleChange} size="sm"/><Form.Control.Feedback type="invalid">{formErrors.Date_Debut}</Form.Control.Feedback></Form.Group>
                             <Form.Group as={Col} md={6} controlId="formDateFin"><Form.Label className="small mb-1 fw-medium">Date Fin</Form.Label><Form.Control className={inputClass} isInvalid={!!formErrors.Date_Fin} type="date" name="Date_Fin" value={formData.Date_Fin} onChange={handleChange} size="sm" min={formData.Date_Debut || undefined}/><Form.Control.Feedback type="invalid">{formErrors.Date_Fin}</Form.Control.Feedback></Form.Group>
                         </Row>

                         {/* --- Fonctionnaire Select Row (V1 structure/style) --- */}
                        <Row className="mb-1 g-3">
                             <Form.Group as={Col} md={12} controlId="formId_Fonctionnaire" id="formId_Fonctionnaire">
                                 <Form.Label className="small mb-1 fw-medium">
                                     <FontAwesomeIcon icon={faUsers} className="me-1 text-secondary"/> Points Focaux
                                     {/* Optional required indicator based on validation */}
                                     {/* {formErrors.id_fonctionnaire && <span className="text-danger">*</span>} */}
                                 </Form.Label>
                                 <Select
                                     inputId='projet-fonctionnaire-select' name="fonctionnaires" menuPlacement="auto"
                                     options={fonctionnairesOptions} value={formData.fonctionnaires} onChange={handleFonctionnaireChange}
                                     styles={selectStyles} placeholder="- Sélectionner -"
                                     isMulti isClearable closeMenuOnSelect={false} isLoading={loadingOptions.fonctionnaires}
                                     className={formErrors.id_fonctionnaire ? 'is-invalid' : ''} classNamePrefix="react-select"
                                     menuPortalTarget={document.body}
                                 />
                                 <Form.Control.Feedback type="invalid" style={{ display: formErrors.id_fonctionnaire ? 'block' : 'none'}}>
                                     {formErrors.id_fonctionnaire}
                                 </Form.Control.Feedback>
                             </Form.Group>
                        </Row>

                         {/* Row 5: Avancement, Coûts (V1 Style) */}
                         <Row className="mb-1 g-3">
                        ...
                        {/* === Engagements Section (Using V1/V2 structure) === */}
            ...
            {/* Confirmation Modal (V2 Structure) */}
...
// --- PropTypes ---
ProjetForm.propTypes = { /* ... same as V1/V2 ... */ itemId: PropTypes.oneOfType([PropTypes.string, PropTypes.number]), onClose: PropTypes.func.isRequired, onItemCreated: PropTypes.func, onItemUpdated: PropTypes.func, baseApiUrl: PropTypes.string, };
// --- Default Props ---
ProjetForm.defaultProps = { /* ... same as V1/V2 ... */ itemId: null, onItemCreated: (createdItem)=>{console.log("Projet Created:",createdItem);}, onItemUpdated: (updatedItem)=>{console.log("Projet Updated:",updatedItem);}, baseApiUrl: 'http://localhost:8000/api', }; // Use /api default
